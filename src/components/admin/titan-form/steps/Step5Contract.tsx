...
import { Copy, CheckCircle, FileCheck, Eye, Loader2 } from "lucide-react";
import { collection, addDoc, serverTimestamp } from "firebase/firestore";
import { db } from "../../../../lib/firebase";
import { useRouter } from "next/navigation";

export default function Step5Contract() {
    const { formData, updateData, prevStep, resetForm } = useTitanStore();
    ...
    const handleFinish = async () => {
        try {
            ...
            alert("Error al guardar el contrato. Por favor intenta nuevamente.");
        } finally {
            setIsSaving(false);
        }
    };
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2 bg-white/5 p-1 rounded-lg">
                                    {['Si', 'No'].map(opt => (
                                        <button
                                            key={opt}
                                            onClick={() => updateData({ exclusividad: opt as 'Si' | 'No' })}
